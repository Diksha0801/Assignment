...
from dbmappper import DbEngine as db
import uuid
...
def add_class_record(params):
    ...
    id = uuid.uuid1()
    class_table = Class(class_id=id, class_name=name, created_on=datetime.now())  # , class_id=class_id)
...
def update_class_by_id(param):
    ...
    class_table = session.query(Class).filter(Class.class_id == id)
...
def delete_class(param):
    ...
    id = param['id']
    print(param)
    session.query(Class).filter(Class.class_id == id).delete()
    session.commit()
...
def update_classleader(param):
    ...
    for data in class_data:
        if data.class_leader is None:
            update_dict[Class.class_leader] = id
...
def add_student_record(params):
    ...
    student = Student(student_id=id, student_name=name, class_id=class_id, created_on=datetime.now())

    session.add(student)
    session.commit()

    return 'update successful', 201, {'c_id':class_id,'s_id':id}
...
def delete_student(param):
    ...
    id = param['id']
    print(param)
    session.query(Student).filter(Student.student_id == id).delete()
    session.commit()
    return ({'result': True},201)
